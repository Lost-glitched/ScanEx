...
export default function App() {
  ...
  const findingCount = results.reduce((sum, result) => sum + (result.baseline?.pii_findings.length || 0) + (result.baseline?.financial_findings.length || 0) + (result.baseline?.redaction_failures.length || 0) + (result.adversarial?.vlm_analysis?.observations.length || 0) + (result.adversarial?.geolocation ? 1 : 0), 0);
